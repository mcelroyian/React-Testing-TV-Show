...
//test seasons show when clicked
test("App renders dropdown loads season", async () => {
  mockFetchShow.mockResolvedValueOnce(episodeData);

  const { queryAllByTestId, getByPlaceholderText, getByText, getAllByText } = render(<App />);
  expect(getByText('Fetching data...')).toBeInTheDocument()

  await waitFor(() => expect(getAllByText('Stranger Things')).toHaveLength(2))
...
// test after click shows exist

// test changing shows works
